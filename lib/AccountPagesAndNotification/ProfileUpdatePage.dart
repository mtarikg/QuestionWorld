...
class _ProfileUpdatePageState extends State<ProfileUpdatePage> {
                ...
                onPressed: () {
                  ...
                  if (oldUserName != userName) {
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                      content: Text(
                        "Your Username has changed from \"$oldUserName\" to \"$userName\"",
                        style: TextStyle(
                            color: Colors.white,
                            fontSize: 16,
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                      content: Text(
                        "Your new UserName is the same as the old one ",
                        style: TextStyle(
                            color: Colors.white,
                            fontSize: 16,
                            fontWeight: FontWeight.bold),
              ...
              MaterialButton(
                ...
                onPressed: () {
                  ...
                  if (instagramUsername != oldInstaUserName) {
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                      content: Text(
                        "Your Instagram Username has changed from \"$oldInstaUserName\" to \"$instagramUsername\"",
                        style: TextStyle(
                            fontWeight: FontWeight.bold,
                            fontSize: 16,
                            color: Colors.white),
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                      content: Text(
                        "Your new Instagram UserName is the same as the old one ",
                        style: TextStyle(
                            fontSize: 16,
                            fontWeight: FontWeight.bold,
                            color: Colors.white),
              ...
              MaterialButton(
                ...
                onPressed: () {
                  ...
                  if (oldSchoolName != schoolName) {
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                      content: Text(
                        "Your School Name has changed from \"$oldSchoolName\" to \"$schoolName\"",
                        style: TextStyle(
                            fontWeight: FontWeight.bold,
                            fontSize: 16,
                            color: Colors.white),
    ...
    if (permissionStatus.isGranted) {
      ...
      if (image != null) {
        ...
        var downloadUrl = await snapshot.ref.getDownloadURL().then((value) {
          ...
          Firestore.instance
              .collection('users')
              .document(widget.profileOwnerId)
  ...
  void updateIUserName() async {
    await Firestore.instance
        .collection('users')
        .document(widget.profileOwnerId)
        .updateData({'instagram': instagramUsername});
  }
